 ...
 *
 * 基本は `/apply` で作った行を `applySurvey` で更新するだけで、
 * `cases.survey_at` が入っているものが「回答済み」になる（schema.sql 末尾）。
 * **例外は LINE のトークン付きで来て申し込みが見つからないとき**で、そのときだけ新しい案件を作る（下記）。
 *
 * ## どの案件かをどう決めるか
 *
 * 上から順に試し、最初に当たったものを使う：
 *   - `code` … `/survey?c=XXXX-XXXX` で来た人（LINE の確認返信のリンク）
 *   - `email` … 申し込み時に入れたメールで本人を特定する
 *   - `t` … 友だち追加の案内に載せた本人確認トークン（2026-09-10）。
 *           その LINE ユーザーに既に紐づいた案件があればそれを使う
 *
 * どれでも見つからず、**`t` が正しければその LINE ユーザーで新しい案件を作る**
 * （申し込みと違うメールを入れた人・申し込まずに友だち追加した人の回答を捨てないため）。
 * `t` も無ければ 404 を返す。**「この申し込みは存在する／しない」が外から分かる**が、
 * 受付番号は推測が難しく、メールは本人しか知らないので許容する。ここを曖昧に 200 で返すと
 * 「送ったのに反映されない」が起き、原因を画面で説明できなくなるほうが損。
 *
 * ## LINE との紐づけ
 *
 * `t` で来て、**案件にも LINE ユーザーにもまだ相手がいなければ**、保存と同時に
 * `cases.line_user_id` を埋める。どちらかが別の相手と紐づき済みなら触らず、運営者に通知する。
 *
 * ## 認証と防御
 *
 * **認証を掛けられない公開 API** なので src/middleware.ts の PUBLIC_API に入れてある。
 * `/api/apply` と違って **Turnstile は掛けない**。LINE のトークから開く画面なので
 * ウィジェットの表示が挟まると離脱するし、行を新しく作れるのは**友だち追加で配った
 * トークンを持っている人だけ**（トークン1つにつき LINE ユーザー1人）なので、
 * 入口より攻撃の旨味が小さい。
    ...
    try {
      c = await createCase(
        ...
        },
        {
          ...
          medium: 'survey',
          campaign: '',
          logBody: 'LINE のアンケートから受付（お申し込みが見つからなかったため新規に作成）',
        },
      );
