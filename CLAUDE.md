...
- 公開後の自動削除フロー（`publish_article.py` 相当）。Annivにはまだ記事の公開先（ブログ機能・CMS等）が未確定のため、当面は手動でKWマスターDB.csvに公開URLを記録する運用（write-article Step 6-5参照）
...
- **記事の公開先は `anniv.gift/media/` のサブディレクトリ運用に決定**（`lp/`配下に静的ブログとして追加。サブドメインは不採用。根拠はメディア戦略.md 8章）。実装はまだ未着手のため、着手時にwrite-article Step 6-5とrewrite-article Step 1の運用を更新すること。
